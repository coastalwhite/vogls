...
pub fn slice_with(
    ...
    if offset.ty().mode == LogicMode::FourValue {
        ...
        match dst.ty.array_size() {
            None => writeln!(f, "{INDENT}{INDENT}{d} = 0;")?,
    ...
    match (src.ty().mode, dst.ty.array_size(), src.ty().array_size()) {
        ...
        (M::FourValue, Some(dst_arr_size), Some(src_arr_size)) => {
            ...
            writeln!(
                f,
                "{INDENT}{INDENT}tv_part_ll_slice({d}, {s}, {o}, {d_size}, {s_size}, {});",
                !fill_with_x
            )?;
            writeln!(
                f,
                "{INDENT}{INDENT}tv_part_ll_slice({d}+{dst_words}, {s}+{src_words}, {o}, {d_size}, {s_size}, false);"
            )?;
            if fill_with_x {
                writeln!(
                    f,
