...
pub fn deadcode_elimination(
    ...
    for tr in &gl.processes[process].regions {
        ...
        while let Some(bb_key) = scratch_stack.pop() {
            ...
            bb.terminator.for_each_non_temporal_bb(|next| {
        ...
        let mut var_stack = Vec::<(bool, VariableKey)>::new();
        for var in &nodes_to_visit {
            ...
            'stack_loop: while let Some((post_visit, var)) = var_stack.pop() {
                ...
                for &(src, dst) in &edges[start..] {
                    ...
                    num_maybe_unused +=
                        usize::from(matches!(dst_state, VariableState::MaybeUnused(_)));
                ...
                var_stack.extend(
                    edges[start..]
                        ...
                        .filter_map(|(_, dst)| {
                            matches!(nodes[dst], VariableState::MaybeUnused(_))
                                .then_some((false, *dst))
                        }),
                );
            }
