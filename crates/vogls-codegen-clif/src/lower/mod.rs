...
impl Compiled {
    pub fn into_design(self, num_regions: u8) -> crate::runtime::ClifDesign {
...
fn wide_addr(b: &mut FunctionBuilder, ptr: Type, heap: Value, loc: WideLoc, word: u32) -> Value {
    match loc {
        ...
        WideLoc::Heap(base) => b.ins().iadd_imm_u(heap, i64::from(base + word) * 8),
    }
}
/// Load word `word` of a wide value.
fn wide_load(b: &mut FunctionBuilder, ptr: Type, heap: Value, loc: WideLoc, word: u32) -> Value {
    match loc {
        WideLoc::Slot(s) => b.ins().stack_load(ptr, I64, s, (word * 8) as i32),
        WideLoc::Heap(base) => {
            let a = b.ins().iadd_imm_u(heap, i64::from(base + word) * 8);
            b.ins().load(I64, mem(), a, 0)
        }
    }
}
/// Store `val` into word `word` of a wide value.
fn wide_store(
    ...
    match loc {
        WideLoc::Slot(s) => {
            b.ins().stack_store(ptr, val, s, (word * 8) as i32);
        }
        WideLoc::Heap(base) => {
            let a = b.ins().iadd_imm_u(heap, i64::from(base + word) * 8);
...
impl<'a> Compiler<'a> {
    ...
    fn lower_instruction(
        ...
        let gets = |b: &mut FunctionBuilder, v: VariableKey| {
        ...
        match instr {
            ...
            Instruction::Binary(dst, BinaryOp::CaseEquality, s1, s2) => {
                let av = get(b, *s1);
                let bv = get(b, *s2);
                let vm = b.ins().icmp(IntCC::Equal, av, bv);
                let m = if is_fv(*s1) {
                    let as_ = gets(b, *s1);
                    let bs = gets(b, *s2);
                    let sm = b.ins().icmp(IntCC::Equal, as_, bs);
                    b.ins().band(vm, sm)
                } else {
                    vm
                };
            ...
            Instruction::Binary(dst, op, s1, s2) => {
                use BinaryOp::{ArithmeticShiftRight, LogicalShiftLeft, LogicalShiftRight, Power};
                // Ops with no inline narrow path go through the wide_binop shim.
                let shim = matches!(*op, Power | ArithmeticShiftRight)
                    || (is_fv(*dst) && matches!(*op, LogicalShiftLeft | LogicalShiftRight));
                if is_real_binop(*op) {
                    let l = get(b, *s1);
                    let r = get(b, *s2);
                    ...
                    let rs2 = gets(b, *s2);
                    let (val, spc) =
                        fv_binary(b, *op, lv, ls, rv2, rs2, sz(*s1), sz(*s2), sz(*dst));
                    b.def_var(vmap[dst], val);
                    b.def_var(spc_map[dst], spc);
                } else {
                    let l = get(b, *s1);
                    let r = get(b, *s2);
                    let out = self.lower_binary(b, *op, l, r, sz(*s1), sz(*s2), sz(*dst));
                    b.def_var(vmap[dst], out);
                }
            }
            ...
            Instruction::SliceImm(dst, src, offset) => {
                ...
                let shifted = if *offset == 0 {
                    ...
                    b.ins().ushr_imm_u(v, *offset as i64)
                };
                let val = maskv(b, shifted, sz(*dst));
                b.def_var(vmap[dst], val);
                if is_fv(*dst) {
                    let s = gets(b, *src);
                    let shs = if *offset == 0 {
                        ...
                        b.ins().ushr_imm_u(s, *offset as i64)
                    };
                    let spc = maskv(b, shs, sz(*dst));
                    b.def_var(spc_map[dst], spc);
                }
            }
            // Variable-offset slice: extract d_size bits of src at bit `index`;
            // out-of-range bits fill with x. dst is always four-value.
            Instruction::Slice(dst, src, index) => {
                let d_size = sz(*dst);
                let s_size = sz(*src);
                let diff = s_size - d_size;
                let m = mask_of(d_size);
                let off = get(b, *index);
                ...
                let sv = get(b, *src);
                let shifted = b.ins().ushr(sv, off);
                let val_raw = b.ins().band_imm_u(shifted, m);
                // in-range spc mask: off <= diff ? m : (m >> (off - diff)).
                let off_le_diff =
                    b.ins()
                ...
                let spc_shifted = b.ins().ushr(mc, over);
                let in_range = b.ins().select(off_le_diff, mc, spc_shifted);
                let spc_raw = if is_fv(*src) {
                    let ss = gets(b, *src);
                    let ss_shifted = b.ins().ushr(ss, off);
                    b.ins().band(ss_shifted, in_range)
                } else {
                    in_range
                };
                // out-of-range (off >= s_size) or unknown offset => all x.
                let off_ge =
                    b.ins()
                        .icmp_imm_u(IntCC::UnsignedGreaterThanOrEqual, off, s_size as i64);
                let zero = b.ins().iconst(I64, 0);
                let val1 = b.ins().select(off_ge, zero, val_raw);
                let val = b.ins().select(off_known, val1, zero);
                let spc1 = b.ins().select(off_ge, zero, spc_raw);
                let spc = b.ins().select(off_known, spc1, zero);
                b.def_var(vmap[dst], val);
                b.def_var(spc_map[dst], spc);
            }
            ...
            Instruction::Probe(dst, signal, offset) => {
                ...
                if mode == LogicMode::TwoValue {
                    ...
                    if s_size <= 32 {
                        ...
                        let (val, spc) =
                            dyn_slice_read(b, val_ptr, Some(base_ptr), offc, one, s_size, size, 0);
                        b.def_var(vmap[dst], val);
                        b.def_var(spc_map[dst], spc);
                    }
                }
            }
            // Variable-offset probe: slice `dst`-size bits out of the signal's
            ...
            Instruction::ProbeSlice(dst, signal, offset) => {
                ...
                let off_known = if is_fv(*offset) {
                    let os = gets(b, *offset);
                    b.ins().icmp_imm_u(IntCC::Equal, os, mask_of(sz(*offset)))
                } else {
                    b.ins().iconst(types::I8, 1)
                };
                if mode == LogicMode::FourValue && s_size <= 32 {
                    // Packed four-value signal: spc | (val << s_size) in one word.
                    // val/spc share the word, so read them with direct shifts and
                    // mask the overhang past s_size to x (spc=0).
                    ...
                    let val_off = b.ins().iadd_imm_u(off, bshift + s_size as i64);
                    ...
                    let spc_i = b.ins().band(spc_raw, inr);
                    let val_i = b.ins().band(val_raw, inr);
                    let oob =
                        b.ins()
                            .icmp_imm_u(IntCC::UnsignedGreaterThanOrEqual, off, s_size as i64);
                    let zero = b.ins().iconst(I64, 0);
                    let v1 = b.ins().select(oob, zero, val_i);
                    let val = b.ins().select(off_known, v1, zero);
                    let s1 = b.ins().select(oob, zero, spc_i);
    ...
    fn emit_fmt_call(
        ...
        for (i, item) in items.iter().enumerate() {
            ...
            let arg_ptr = if size > 64 {
                wide_addr(b, self.ptr, params.heap_ptr, wide_map[item], 0)
            } else {
                if !fv {
    ...
    fn first_val_word(
        ...
        if size > 64 {
            ...
            wide_load(b, self.ptr, heap, wide_map[&key], off)
        } else {
            b.use_var(vmap[&key])
        }
    }
    ...
    fn emit_wide_binop(
        ...
        let dst_ptr = wide_addr(b, self.ptr, params.heap_ptr, dst_slot, 0);
        ...
        if !dst_wide {
            if is_fv {
                let spc = wide_load(b, self.ptr, params.heap_ptr, dst_slot, 0);
                let val = wide_load(b, self.ptr, params.heap_ptr, dst_slot, 1);
                b.def_var(spc_map[&dst], spc);
                b.def_var(vmap[&dst], val);
            } else {
    ...
    fn lower_binary(
        ...
        match op {
            ...
            OrNot => {
                let nb = b.ins().bnot(r);
                let o = b.ins().bor(l, nb);
                maskv(b, o, dsize)
            }
            Xnor => {
                let x = b.ins().bxor(l, r);
                let n = b.ins().bnot(x);
                maskv(b, n, dsize)
            }
            Add => {
                let a = b.ins().iadd(l, r);
                maskv(b, a, dsize)
            }
            Sub => {
                let a = b.ins().isub(l, r);
                maskv(b, a, dsize)
            }
            Multiply => {
                let a = b.ins().imul(l, r);
                maskv(b, a, dsize)
            }
            DivideX | Divide0 => safe_udiv(b, l, r),
            ModulusX | Modulus0 => safe_urem(b, l, r),
            UnsignedLessEqual => {
                let c = b.ins().icmp(IntCC::UnsignedLessThanOrEqual, l, r);
                b.ins().uextend(I64, c)
            }
            LogicalShiftLeft => {
                let sh = b.ins().ishl(l, r);
                let m = maskv(b, sh, dsize);
                guard_shift(b, r, m)
            }
            LogicalShiftRight => {
                let sh = b.ins().ushr(l, r);
                guard_shift(b, r, sh)
            }
            Concat => {
                let hi = b.ins().ishl_imm_u(l, s2 as i64);
                let o = b.ins().bor(hi, r);
                maskv(b, o, dsize)
            }
            Min => b.ins().umin(l, r),
            Max => b.ins().umax(l, r),
            // Two-value CopyX/CopyZ have no x/z to inject: just copy the value.
            CopyX | CopyZ => maskv(b, l, dsize),
            // Edge detect (two-value): posedge = new & ~old, negedge = old & ~new.
            Posedge => {
                let nl = b.ins().bnot(l);
                let d = b.ins().band(r, nl);
                maskv(b, d, dsize)
            }
            Negedge => {
                let nr = b.ins().bnot(r);
                let d = b.ins().band(l, nr);
                maskv(b, d, dsize)
            }
            _ => {
                b.ins().trap(TRAP_UNIMPLEMENTED);
    ...
    /// Reals are f64 bit patterns in one i64. Native f64 for arithmetic /
    /// compare; the `real_op` shim for transcendentals.
    fn lower_real_binary(
        &self,
        b: &mut FunctionBuilder,
        params: &Params,
        op: BinaryOp,
        ...
        use BinaryOp::*;
        let cmp = |b: &mut FunctionBuilder, cc: FloatCC, fl: Value, fr: Value| {
            let c = b.ins().fcmp(cc, fl, fr);
            b.ins().uextend(I64, c)
        };
        match op {
            RealAdd | RealSub | RealMul | RealDiv => {
                let fl = b.ins().bitcast(F64, cast(), l);
                let fr = b.ins().bitcast(F64, cast(), r);
                let x = match op {
                    RealAdd => b.ins().fadd(fl, fr),
                    RealSub => b.ins().fsub(fl, fr),
                    RealMul => b.ins().fmul(fl, fr),
                    _ => b.ins().fdiv(fl, fr),
                };
                b.ins().bitcast(I64, cast(), x)
            }
            RealEq | RealNe | RealLt | RealLeq | RealGt | RealGeq => {
                let fl = b.ins().bitcast(F64, cast(), l);
                let fr = b.ins().bitcast(F64, cast(), r);
                let cc = match op {
                    RealEq => FloatCC::Equal,
                    RealNe => FloatCC::NotEqual,
                    RealLt => FloatCC::LessThan,
                    RealLeq => FloatCC::LessThanOrEqual,
                    RealGt => FloatCC::GreaterThan,
                    _ => FloatCC::GreaterThanOrEqual,
                };
                cmp(b, cc, fl, fr)
            }
            RealPow => self.real_shim(b, params, rc::POW, l, r),
            RealATan2 => self.real_shim(b, params, rc::ATAN2, l, r),
            RealHypot => self.real_shim(b, params, rc::HYPOT, l, r),
            _ => {
                b.ins().trap(TRAP_UNIMPLEMENTED);
                b.ins().iconst(I64, 0)
            }
    ...
    fn lower_wide_instruction(
        ...
        let rv = |b: &mut FunctionBuilder, v: VariableKey, i: u32| -> Value {
            ...
            if size > 64 {
                ...
                wide_load(b, ptr, heap, wide_map[&v], off)
            } else {
                b.use_var(vmap[&v])
            }
        };
        ...
        match instr {
            Instruction::Constant(dst, bits) => {
                ...
                match bits.as_data_ref() {
                    vogls_bits::BitsDataRef::SeparateTv(w) => {
                        // Two-value-data constant into a (possibly) four-value
                        // dst: write the value words, and for a four-value dst
                        // also mark the whole value KNOWN (spc = all-ones, masked
                        // in the top word). Without the spc writes the spc plane
                        // stays 0 => the constant reads back as all-x. Mirrors the
                        // `InlineTv` arm below.
                        for i in 0..n as usize {
                            let c = b.ins().iconst(I64, w[i] as i64);
                            wv(b, *dst, i as u32, c);
                            if fv {
                                let m = if i == n as usize - 1 {
                                    top_i64(sz(*dst))
                                } else {
                                    -1
                                };
                                let sc = b.ins().iconst(I64, m);
                                ws(b, *dst, i as u32, sc);
                            }
                        }
                    }
                    vogls_bits::BitsDataRef::SeparateFv(w) => {
                        for i in 0..n as usize {
                            let sc = b.ins().iconst(I64, w[i] as i64);
                            ws(b, *dst, i as u32, sc);
                            let vc = b.ins().iconst(I64, w[n as usize + i] as i64);
                            wv(b, *dst, i as u32, vc);
                        }
                    }
                    vogls_bits::BitsDataRef::InlineTv(v) => {
                        for i in 0..n {
                            let c = b.ins().iconst(I64, if i == 0 { v as i64 } else { 0 });
                            wv(b, *dst, i, c);
                            if fv {
                                let m = if i == n - 1 { top_i64(sz(*dst)) } else { -1 };
                                let sc = b.ins().iconst(I64, m);
                                ws(b, *dst, i, sc);
                            }
                        }
                    ...
                    vogls_bits::BitsDataRef::InlineFv(spc, val) => {
                        for i in 0..n {
                            let vc = b.ins().iconst(I64, if i == 0 { val as i64 } else { 0 });
                            wv(b, *dst, i, vc);
                            let sc = b.ins().iconst(I64, if i == 0 { spc as i64 } else { 0 });
                            ws(b, *dst, i, sc);
                        }
                    }
                }
            ...
            Instruction::Probe(dst, signal, off) => {
                ...
                if *off == 0 && d_size == s_size {
                    ...
                    for i in 0..words {
                        ...
                        wide_store(b, ptr, heap, wide_map[dst], i, w);
                    }
                } else {
            ...
            Instruction::Slice(dst, src, index) => {
                ...
                let (val_ptr, spc_ptr) = if is_fv(*src) {
                    let vp = wide_addr(b, ptr, heap, loc, src_nwords);
                    let sp = wide_addr(b, ptr, heap, loc, 0);
                    (vp, Some(sp))
                } else {
                    let vp = wide_addr(b, ptr, heap, loc, 0);
                    (vp, None)
                };
                let (val, spc) =
            ...
            Instruction::Unary(dst, UnaryOp::TvToFv, src) => {
                let n = nwords(sz(*dst));
                for i in 0..n {
                    let v = rv(b, *src, i);
                    wv(b, *dst, i, v);
            ...
            Instruction::Unary(dst, UnaryOp::FvToTv, src) => {
                let n = nwords(sz(*dst));
                for i in 0..n {
                    let v = rv(b, *src, i);
                    let s = rs(b, *src, i);
            ...
            Instruction::Unary(dst, UnaryOp::Neg, src) => {
                let n = nwords(sz(*dst));
                let fv = is_fv(*dst);
                for i in 0..n {
                    let sv = rv(b, *src, i);
                    if fv {
                        ...
                        let mut val = b.ins().band(ss, nsv);
                        let mut spc = ss;
                        if i == n - 1 {
                            val = b.ins().band_imm_u(val, top_i64(sz(*dst)));
                            spc = b.ins().band_imm_u(spc, top_i64(sz(*dst)));
                        }
                        wv(b, *dst, i, val);
                        ws(b, *dst, i, spc);
                    } else {
                        let mut w = b.ins().bnot(sv);
                        if i == n - 1 {
                            w = b.ins().band_imm_u(w, top_i64(sz(*dst)));
                        }
                        wv(b, *dst, i, w);
                    }
                }
                ...
                if matches!(op.as_ref(), IntrinsicOp::BlackBox) =>
            {
                ...
                let fv = is_fv(*dst);
                for i in 0..n {
                    let v = rv(b, items[0], i);
                    wv(b, *dst, i, v);
                    if fv {
                        let s = rs(b, items[0], i);
                        ws(b, *dst, i, s);
                    }
                }
            }
    ...
    fn emit_wide_slice(
        ...
        for i in 0..dst_words {
            wide_store(b, self.ptr, params.heap_ptr, dst_slot, i as u32, z);
        }
        b.ins().jump(merge, &[]);
        b.switch_to_block(merge);
...
fn fv_binary(
    ...
    match op {
        ...
        Add => {
            let raw = b.ins().iadd(lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        Sub => {
            let raw = b.ins().isub(lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        Multiply => {
            let raw = b.ins().imul(lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        DivideX => {
            let nz = b.ins().icmp_imm_u(IntCC::NotEqual, rv, 0);
            let one = b.ins().iconst(I64, 1);
            let safe = b.ins().select(nz, rv, one);
            let q = b.ins().udiv(lv, safe);
            fv_arith_core(b, q, ls, rs, s1, s2, Some(nz))
        }
        Divide0 => {
            let raw = safe_udiv(b, lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        ModulusX => {
            let nz = b.ins().icmp_imm_u(IntCC::NotEqual, rv, 0);
            let one = b.ins().iconst(I64, 1);
            let safe = b.ins().select(nz, rv, one);
            let q = b.ins().urem(lv, safe);
            fv_arith_core(b, q, ls, rs, s1, s2, Some(nz))
        }
        Modulus0 => {
            let raw = safe_urem(b, lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        UnsignedLessEqual => {
            let bk = both_known(b, ls, s1, rs, s2);
            let cmp = b.ins().icmp(IntCC::UnsignedLessThanOrEqual, lv, rv);
        ...
        Concat => {
            let vh = b.ins().ishl_imm_u(lv, s2 as i64);
            let v0 = b.ins().bor(vh, rv);
            let val = maskv(b, v0, dsize);
            let sh = b.ins().ishl_imm_u(ls, s2 as i64);
            let s0 = b.ins().bor(sh, rs);
            let spc = maskv(b, s0, dsize);
            (val, spc)
        }
        Min => {
            let raw = b.ins().umin(lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        Max => {
            let raw = b.ins().umax(lv, rv);
            fv_arith_core(b, raw, ls, rs, s1, s2, None)
        }
        CopyX => {
...
#[cfg(test)]
mod tests {
    use super::{SignalInfo, compile};
    use vogls_codegen::HeapBuilder;
    use vogls_ir::time::TimeResolution;
    use vogls_ir::token_range::TokenRange;
    use vogls_ir::{
        BasicBlock, BasicBlockTerminator, GlobalContext, Process, ProcessKind, TemporalRegionKey,
        Time,
    };
    use vogls_runtime::{RuntimeState, SimulationIo, TimeFormat};
    use vogls_utils::VgHashMap;

    fn empty_info() -> (
        VgHashMap<vogls_ir::SignalKey, vogls_runtime::RtSignalKey>,
        VgHashMap<vogls_runtime::RtSignalKey, u64>,
    ) {
        (VgHashMap::default(), VgHashMap::default())
    }

    fn run(gl: &GlobalContext, max_time: u64) {
        let (rt_map, lupdt) = empty_info();
        let info = SignalInfo {
            signal_to_heap: &[],
            rt_signal_map: &rt_map,
            signal_mode: &[],
            lupdt_indexes: &lupdt,
        };
        ...
        let runtime = RuntimeState::new(gl, heap, &[], &[], time_format);
        let mut state = design.new_state(compiled_listeners(), 3, runtime, gl);
        let mut io = SimulationIo::default();
        design.run(&mut state, &mut io, max_time).unwrap();
    }

    // Empty designs have no listeners.
    fn compiled_listeners() -> usize {
        0
    }

    fn empty_bb(gl: &mut GlobalContext, term: BasicBlockTerminator) -> TemporalRegionKey {
        let key = gl.bbs.insert_with_key(|k| BasicBlock {
            instrs: Vec::new(),
            region: TemporalRegionKey::from_entry(k),
            terminator: term,
        });
        TemporalRegionKey::from_entry(key)
    }

    fn one_process(gl: &mut GlobalContext, regions: Vec<TemporalRegionKey>) {
        gl.processes.insert(Process {
    ...
    #[test]
    fn empty_design_runs() {
        run(&GlobalContext::default(), u64::MAX);
    }

    #[test]
    fn single_tr_halt() {
        let mut gl = GlobalContext::default();
        let tr = empty_bb(&mut gl, BasicBlockTerminator::Halt);
        one_process(&mut gl, vec![tr]);
        run(&gl, u64::MAX);
    }

    #[test]
    fn wait_delay_reschedules() {
        let mut gl = GlobalContext::default();
        let key = gl.bbs.insert_with_key(|k| {
            let tr = TemporalRegionKey::from_entry(k);
            BasicBlock {
                instrs: Vec::new(),
                region: tr,
                terminator: BasicBlockTerminator::Wait(tr, Time(1)),
            }
        });
        let tr = TemporalRegionKey::from_entry(key);
        one_process(&mut gl, vec![tr]);
        run(&gl, 3);
    }

    #[test]
    fn wait_zero_chains_trs() {
        let mut gl = GlobalContext::default();
        let tr1 = empty_bb(&mut gl, BasicBlockTerminator::Halt);
        let tr0 = empty_bb(&mut gl, BasicBlockTerminator::Wait(tr1, Time(0)));
        one_process(&mut gl, vec![tr0, tr1]);
        run(&gl, u64::MAX);
    }
}
