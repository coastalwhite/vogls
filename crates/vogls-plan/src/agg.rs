...
/// Describes transforms from 0+ values to 1 value.
///
/// This trait is used to describe how to do aggregations on `Array` and vertical aggregations on
/// `RunVector` in an abstract sense.
pub trait Agg: std::hash::Hash + Eq + Clone + Send + Sync + 'static {
    ...
    /// Format the information relevant to the aggregation.
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result;

    /// Get the output type for the aggregation given the input types.
