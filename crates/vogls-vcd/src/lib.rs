...
use vogls_ir::{Bits, LogicMode, SCALAR_VSIZE, SignalKey, SignalSlice, VectorSize};

pub struct RtVcdOutput {
    pub printed_header: bool,
    pub start_ts: u64,
    pub last_ts: u64,
    pub time_scale: TimeResolution,
    pub paused: bool,
...
fn dump_time_step(
    ...
    if !*printed_header {
        ...
        writeln!(
            f,
            "$timescale {}{} $end",
            time_scale.size.as_str(),
            time_scale.unit.as_str()
        )?;
        for child in children {
            child.write_to(f)?;
        }
    ...
    *last_ts = time;
    ...
    for signal in updated_this_time_step.iter() {
        let signal_idx = signal.as_usize();
        let bits = signals[signal_idx];
        let mode = signal_mode[signal_idx];
        let bits = heap.load_bits(bits, mode);
