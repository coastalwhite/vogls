module add1 ( input a, input b, input cin, output add1sum, output cout );
    assign { cout, add1sum } = { 1'b0, a } + { 1'b0, b } + cin;
endmodule

module add16 ( input [15:0] a, input[15:0] b, input cin, output [15:0] sum, output cout );
    wire couts [14:0];
    add1 i1 ( .a(1'b1), .b(1'b1), .cin(1'b0), .add1sum(sum[ 0]), .cout() );
    add1 i2 ( .a(1'b0), .b(1'b0), .cin(1'b1), .add1sum(sum[ 1]), .cout() );
    // add1 i3 ( .a(1'b0), .b(1'b0), .cin(1'b0), .sum(), .cout() );
...
module top_module ( input [31:0] a, input [31:0] b, output [31:0] sum );
    ...
    add16 lo ( .a(16'h1),  .b(16'h1),  .cin(1'b0),    .sum(sum[15:0]),  .cout(lo_cout) );
