...
enum ConstantPartSelectError {
    ResolveFail,
    NotANet,
    ArrayOutput,
}

enum SignalInteractKind {
    Probe,
    Drive,
}

pub fn constant_eval_part_select<'a>(
    ctx: &LowerContext<'a>,
    mctx: &mut MutLowerContext,
    ...
    ident: HIdent<'a>,
    exprs: AstIdRange<'a, Expr<'a>>,
    range_expression: Option<BitSlice<'a>>,

    signal_interact_kind: SignalInteractKind,
) -> Result<(SignalKey, SignalSlice), ConstantPartSelectError> {
    let Some(net) = resolve_hident(
        scope,
        &ctx.table,
        ident,
    ) else {
        return Err(ConstantPartSelectError::ResolveFail);
    };
    let VSymbol::Net(net) = &ctx.table[net].content else {
        return Err(ConstantPartSelectError::NotANet);
    };

    let NetSymbol { ty, dims, lsb, bit_reversed, net, port_idx } = &net;
    let leaf_width = ty.force_net_width();
    let signal = match signal_interact_kind {
        SignalInteractKind::Probe => net.probe_signal(),
        SignalInteractKind::Drive => net.blocking_drive_signal(),
    };

    // Fast path: No array and no indexing.
    if dims.is_empty() && exprs.is_empty() && range_expression.is_none() {
        return Ok((signal, SignalSlice::with_end(leaf_width)));
    }
    ...
    //
    // a[1][2]      = a + (1* (5*3)) + (2*(5))
    // a[0][1][3:1] = a + (0* (5*3)) + (1*(5)) + 1
    
    let (pre_range_width, bit = match exprs.len().cmp(&dims.len()) {
        Ordering::Less => {
            return Err(ConstantPartSelectError::ArrayOutput)
        },
        Ordering::Equal => leaf_width,
        Ordering::Greater => SCALAR_VSIZE,
    };

    let (mut lsb, width) = match range_expression {
        None => (0, pre_range_width),
        Some(BitSlice::MsbLsb(msb, lsb)) => {
        },
        Some(BitSlice::PlusWidth(offset, width)) => {
        },
        Some(BitSlice::MinusWidth(offset, width)) => {
        },
    };

    let bit_indices = 

    for expr in exprs.rev().take(exprs.len().saturating_sub(dims.len())) {
